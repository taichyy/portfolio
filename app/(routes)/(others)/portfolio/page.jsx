...
const Portfolio = () => {
    ...
    let allBadges = []
    portfolio.map(item => allBadges.push(...item.badges))
                    ...
                    {uniqueBadges.map(badge => (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                                {portfolio.filter(item => item.badges.includes(badge)).map((item, index) => (
                                    <Link key={index} href={`/portfolio/${item.id}`}>
                                        <Card className="group h-full overflow-hidden transition-all duration-300 hover:shadow-lg hover:-translate-y-2 hover:border-primary/50 cursor-pointer bg-card/50 backdrop-blur-sm border border-border/50 hover:bg-card flex flex-col">
                                            {/* Image Container */}
